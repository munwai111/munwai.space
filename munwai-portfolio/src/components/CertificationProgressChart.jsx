...
const CertificationProgressChart = ({ deviceInfo = {} }) => {
  ...
  // Animate progress with requestAnimationFrame when in view
  useEffect(() => {
    let frame;
    let start;
    const duration = 1200; // ms
    let observer;
    let chartElement = document.getElementById("certification-chart");
    ...
    // Use IntersectionObserver to trigger animation on scroll-in
    ...
    return () => {
      ...
      if (observer && chartElement) observer.disconnect();
    };
  }, [data]);

  // Interpolate Y from bottom to real value
  const getPointPosition = (index, value, prog = progress) => {
    const plotWidth = chartWidth - marginLeft - marginRight;
    const plotHeight = chartHeight - marginTop - marginBottom;
    const x = marginLeft + (index / (data.length - 1)) * plotWidth;
    const yTarget = marginTop + plotHeight - (value / maxValue) * plotHeight;
    const y = yTarget * prog + (1 - prog) * (marginTop + plotHeight);
    return { x, y };
  };

  // Generate path using progress
  const generatePath = () => {
    if (data.length === 0) return "";
    let path = "";
    data.forEach((point, index) => {
      const pos = getPointPosition(index, point.cumulative, progress);
      if (index === 0) {
        path += `M ${pos.x} ${pos.y}`;
      } else {
        const prevPos = getPointPosition(
          index - 1,
    ...
    >
      ...
      >
        <div className="text-center mb-8">
          ...
          >
            Continuous professional development through certifications and skill
            building
          </p>
        </div>
        ...
        >
          <svg
            ...
            style={{
              minWidth: deviceInfo.isFlipPhone
                ? "280px"
                : deviceInfo.isMobile
                ? "350px"
                : deviceInfo.isTablet
          ...
          >
            ...
            {[0, 5, 10, 15, 20, 22].map((value) => {
              ...
              const plotHeight = chartHeight - marginTop - marginBottom;
              const y =
                marginTop + plotHeight - (value / maxValue) * plotHeight;
              return (
                <g key={value}>
                  <line
                    x1={marginLeft}
            ...
            <g clipPath="url(#chartClip)">
              ...
              <path
                ...
                } ${plotAreaBottom} L ${marginLeft} ${plotAreaBottom} Z`}
        ...
        >
          <div className="flex items-center gap-2">
            <div
              className={`bg-gradient-to-r from-blue-500 to-cyan-500 rounded-full shadow-lg ${
                deviceInfo.isFlipPhone
                  ? "w-3 h-3"
                  : deviceInfo.isMobile
                  ? "w-4 h-4"
          ...
          <div className="flex items-center gap-2">
            <div
              className={`bg-blue-500/30 rounded-full shadow-lg ${
                deviceInfo.isFlipPhone
                  ? "w-3 h-3"
                  : deviceInfo.isMobile
                  ? "w-4 h-4"
